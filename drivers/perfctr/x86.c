...
/* Just implementing a mode now, APIC not compatible with
 * Linux, comment it firstly for a mode implementation
 */
...
/* lwk has no hw_irq head file, need to check with author 
 * how it handles the interrupt, also need to figure out
 * why use it in perfctr
 *
 * temporary created a new file define perfctr vector under 
 * linux/include/asm
 *
 */
#include <asm/hw_irq.h>

/* lwk seems didnot define cpu_khz
 * need to figure out what it is for
 * and who initiate it 
 */
...
/* 
 * define the address of MSRS, and control flags
 * classified with different architecture
 */
...
/* Intel P6, VIA C3 */
#define MSR_P6_PERFCTR0		0xC1		/* .. 0xC2, IA32_PMC address */
#define MSR_P6_EVNTSEL0		0x186		/* .. 0x187 IA32_PERFEVTSEL address*/
...
static int __init p4_ht_smp_init(void)
{
	...
	on_each_cpu(p4_ht_mask_setup_cpu, &forbidden, 1);
...
static int __init intel_init(void)
{
	...
	if (!cpu_has_tsc)
		return -ENODEV;
	switch (current_cpu_data.x86_family) {
	case 5:
		if (cpu_has_mmx) {
...
{
#if 0 // don't consider other archtecture now, as no cpu_core_map data structure */
	cpumask_t non0cores;
	int i;
...
#if LWK_VERSION_CODE >= KERNEL_VERSION(2,6,19)
...
{
#if LWK_VERSION_CODE >= KERNEL_VERSION(2,6,21)
...
static int perfctr_reserve_lapic_nmi(void)
{
	...
	if (atomic_read(&nmi_active) <= 0)
		return 0;
#if LWK_VERSION_CODE >= KERNEL_VERSION(2,6,22)
