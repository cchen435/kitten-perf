...
#if LWK_VERSION_CODE >= KERNEL_VERSION(2,6,18)
static int
...
#if LWK_VERSION_CODE < KERNEL_VERSION(2,6,19) && !DONT_HAVE_i_blksize
	inode->i_blksize = 0;
#endif
	return inode;
}
...
static struct task_struct *vperfctr_get_tsk(struct vperfctr *perfctr)
{
	struct task_struct *tsk;
	tsk = current;
	if (perfctr != current->arch.thread.perfctr) {
